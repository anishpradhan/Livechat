...
import { wsConnect, wsDisconnect, newMessage, hasRead } from "../../modules/websocket";
...
const ChatWindow = () => {
  ...
  const connectAndJoin = () => {
    const host = `${process.env.REACT_APP_SOCKET_URL}/${chatID}/`;
  ...
  useEffect(() => {
    // el.current.scrollIntoView({block: 'end', behaviour:'smooth'});
  ...
  let messageChats = messages.map((message, index) => {
    const renderTimestamp = (timestamp) => {
      ...
      if (timeDiff < 24 * 60) {
        ...
        const dateTime = new Intl.DateTimeFormat('en-US', {hour: '2-digit', minute: '2-digit'}).format(new Date(timestamp));
        prefix = `${dateTime}`;
    ...
    if (message.type === 'info'){
      return (<ChatInfo key={message.id} time={renderTimestamp(message.sent)} info={message.message} />);  
    }
    // else if (message.type === 'started_info'){
    //   return (<ChatStart key={message.id} time={renderTimestamp(message.sent)} />);  
      ...
      <div className="h-full grow flex flex-col items-stretch">
        ...
        <div className="h-5/6 w-full flex flex-col-reverse p-3 space-y-[1px] overflow-y-auto">
          {!agentExists ? (
            <div className="py-4">
              <button
                onClick={() => dispatch(joinChat(chatID))}
          ...
          {/* <RecieveMessage username={username}
          message='...'
          receiveMessageContinue={true}
          lastMessage={true}
          samePreviousReceiver={false} /> */}
          {isTyping?<div className={`flex items-top space-x-2 space-y-[0px]`}>
          <span className={`w-8 h-8 shrink-0 rounded-full border-4 bg-gray-200`}>
          <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <span className="text-sm text-gray-500">{username}</span>
            <span className={`px-3 py-1 flex-shrink bg-[#F2F3F4] rounded-l-md rounded-r-3xl text-left rounded-bl-3xl`}>
            <svg
            xmlns="http://www.w3.org/2000/svg"
